classdef DiffDriveFollower < control
    ...
    properties
        type % [d,d] or [d,phi] formation  对应两种不同的控制模式 d-d  或者是 d-phi
        d1
        d2
        d
        phi
    ...
    methods
        function obj = DiffDriveFollower(type,params)
            ...
            obj.type = type;
            if (strcmp(type,"dd")) % distance-distance formation
                obj.d1 = params.d1;
                obj.d2 = params.d2;
            elseif (strcmp(type,"dphi"))% distance-phi formation
                obj.d = params.d;
                obj.phi = params.phi;
            end
        end
        
        function control = compute_control(obj,pose,lead1,lead2) %lead2 在d-phi中是不需要的
            % compute the control according to current position
            if (strcmp(obj.type,"dphi"))
                control = obj.compute_dphi(pose,lead1,obj.d,obj.phi); % distance-phi formation pose是自己的pose，d-phi
            elseif (strcmp(obj.type,"dd"))
                control = obj.compute_dd(pose,lead1,lead2); % distance-distance formation
            end
        end
