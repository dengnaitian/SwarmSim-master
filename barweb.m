...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	% Plot bars
	handles.bars = barh(barvalues, width,'edgecolor','k', 'linewidth', 2);
	h= handles.bars;
    set(h(1),'FaceColor','#5D6BC3');
    set(h(2),'facecolor','#8FCD79');
    
    ytips1 = h(1).XEndPoints
    xtips1 = h(1).YEndPoints + 0.03
    labels1 = string(h(1).YData) + '¡À' + errors(1:4,1)'
    text(xtips1,ytips1,labels1,'HorizontalAlignment','center',...
        'VerticalAlignment','bottom')
    ytips2 = h(2).XEndPoints;
    xtips2 = h(2).YEndPoints + 0.03;
    labels2 = string(h(2).YData) + '¡À' + errors(1:4,2)';
...
% 		handles.errors(i) = errorbar(x, barvalues(:,i), errors(:,i),'horizontal', 'k', 'linestyle', 'none', 'linewidth', 2);
        handles.errors(i) = errorbar(barvalues(:,i),x, errors(:,i),'horizontal', 'k', 'linestyle', 'none', 'linewidth', 2);
	...
	xlim([0 ymax*1.1]);
	ylim([0.5 numgroups-change_axis+0.5]);
	...
	set(gca, 'yticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'fontsize', 12, 'ytick',1:numgroups, 'linewidth', 2,'xgrid','off','ygrid','off');
