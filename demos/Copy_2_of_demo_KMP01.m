...
load('D:\CC\robInfLib-matlab-master\2Dletters\G.mat');
...
[DataOut, SigmaOut,H] = GMR(model, [first:last]*model.dt, 1, 2:model.nbVar); %see Eq. (17)-(19)

for i=1:nbData
    refTraj(i).t=first*model.dt+ (i-1)*model.dt;
...
viaNum=4;
via_time(1)=dt;
via_point(:,1)=[8 6 -50 0]'; % format:[2D-pos 2D-vel]
via_time(2)=0.25;
...
for index=first:last
    ...
    kmpPredTraj(index-first+1).t=index*dt;
